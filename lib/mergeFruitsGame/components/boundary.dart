...
class Bucket extends BodyComponent {
  ...
  static double bucketWidth = 25;

  final Paint wallPaint = Paint()..color = Color(0xff444753);
  ...
  Body createBody() {
    ...
    final leftWallShape = PolygonShape()
      ..setAsBox(
        wallWidth,
        bucketHeight / 2,
        Vector2(-bucketWidth / 2, (bucketHeight / 2) + 4),
        0,
      );
    ...
    final bottomWallShape = PolygonShape()
      ..setAsBox(bucketWidth / 2, wallWidth, Vector2(0, bucketHeight + 4), 0);
    body.createFixture(
      FixtureDef(bottomWallShape)
        ..density = 100
        ..friction = 1,
    );
  ...
  void render(Canvas canvas) {
    ...
    if (leftWall.shape is PolygonShape) {
      ...
      final path = Path()
        ..addPolygon(vertices.map((v) => Offset(v.x, v.y)).toList(), true);
    ...
    canvas.drawCircle(
      Offset(-bucketWidth / 2, bucketHeight + 4),
      wallWidth,
      wallPaint,
    );
    canvas.drawCircle(
    ...
    canvas.drawCircle(Offset(-bucketWidth / 2, 4), wallWidth, wallPaint);
    canvas.drawCircle(Offset(bucketWidth / 2, 4), wallWidth, wallPaint);
  }
}
